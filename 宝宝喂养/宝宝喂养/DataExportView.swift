...
struct DataExportView: View {
    ...
    var exportBabyName: String { babyProfile?.name ?? "宝宝" }
    ...
    var dateRangeText: String {
        guard let first = records.min(by: { $0.timestamp < $1.timestamp }),
              let last = records.max(by: { $0.timestamp < $1.timestamp }) else { return "无数据" }
    ...
    func exportData() {
        ...
        let fileName = exportFormat == 0 ? "\(safeFileName(exportBabyName))-宝宝记录.csv" : "\(safeFileName(exportBabyName))-宝宝记录.json"
    ...
    func generateCSV() -> String {
        ...
        for record in records.sorted(by: { $0.timestamp < $1.timestamp }) {
            ...
            let row = [exportBabyName, type, time, record.note, record.displaySummary]
                .map(csvField)
                .joined(separator: ",")
            csv += "\(row)\n"
        }
