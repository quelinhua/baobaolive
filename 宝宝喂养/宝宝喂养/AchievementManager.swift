...
enum AchievementCategory: String, Codable, CaseIterable {
    ...
    var icon: String {
        ...
        case .feeding: return RecordType.feeding.iconName
...
class AchievementManager {
    ...
    static let allAchievements: [AchievementDef] = [
        ...
        // 喂奶类
        AchievementDef(id: "first_feeding", name: "第一口奶", desc: "记录第一次喂奶", category: .feeding, level: .bronze, icon: RecordType.feeding.iconName, requirement: 1),
    ...
    func checkAchievements(records: [RecordModel]) {
        ...
        setProgress("feeding_8times", value: feedingToday)
        ...
        var checkDate = recordDays.contains(today) ? today : calendar.date(byAdding: .day, value: -1, to: today)!
        ...
        checkDate = feedingDays.contains(today) ? today : calendar.date(byAdding: .day, value: -1, to: today)!
        ...
        checkDate = sleepDays.contains(today) ? today : calendar.date(byAdding: .day, value: -1, to: today)!
    ...
    enum AchievementRank: Int, CaseIterable {
        case newbie = 0
        case skilled = 1
        case expert = 2
        case master = 3
        ...
        var displayName: String {
            switch self {
            case .newbie: return "记录新手"
            case .skilled: return "记录能手"
            case .expert: return "记录达人"
            case .master: return "记录大师"
            case .legend: return "记录传奇"
            }
        }
        ...
        var gradient: [Color] {
            switch self {
            case .newbie: return [Color(hex: "CD7F32"), Color(hex: "8B5E3C")]
            case .skilled: return [Color(hex: "C0C0C0"), Color(hex: "A8A8A8")]
            case .expert: return [Color(hex: "FFD700"), Color(hex: "FFA500")]
            case .master: return [Color(hex: "FFD700"), Color(hex: "FF8C00")]
            case .legend: return [Color(hex: "B9F2FF"), Color(hex: "7DF9FF"), Color(hex: "FFD700")]
            }
        }

        var shadowColor: Color {
            switch self {
            case .newbie: return Color(hex: "CD7F32").opacity(0.3)
            case .skilled: return Color(hex: "C0C0C0").opacity(0.3)
            case .expert: return Color(hex: "FFD700").opacity(0.3)
            case .master: return Color(hex: "FFD700").opacity(0.4)
            case .legend: return Color(hex: "7DF9FF").opacity(0.4)
            }
        }
    }

    var currentRank: AchievementRank {
        let count = unlockedCount
        switch count {
        case 0...2: return .newbie
        case 3...6: return .skilled
        case 7...12: return .expert
        case 13...18: return .master
